...
macro_rules! protocol_tests {
    ($suite:ty, $mod_name:ident) => {
        mod $mod_name {
            use super::*;
            use shortcake::{Error, Initiator, Responder};
            ...
            #[test]
            fn commitment_mismatch_wrong_msg3() {
                let mut rng = test_rng();
                let (init_a, msg1_a) = Initiator::<$suite>::start(&mut rng);
                let (resp_a, msg2_a) = Responder::<$suite>::start(&mut rng, msg1_a).unwrap();
                let (_out_a, _msg3_a) = init_a.finish(msg2_a).unwrap();

                let (init_b, msg1_b) = Initiator::<$suite>::start(&mut rng);
                let (_resp_b, msg2_b) = Responder::<$suite>::start(&mut rng, msg1_b).unwrap();
            ...
            #[test]
            fn cross_session_sas_differs() {
                let mut rng = test_rng();

                let (init1, msg1a) = Initiator::<$suite>::start(&mut rng);
                let (resp1, msg2a) = Responder::<$suite>::start(&mut rng, msg1a).unwrap();
                let (out1_i, msg3a) = init1.finish(msg2a).unwrap();
                let out1_r = resp1.finish(msg3a).unwrap();

                let (init2, msg1b) = Initiator::<$suite>::start(&mut rng);
                let (resp2, msg2b) = Responder::<$suite>::start(&mut rng, msg1b).unwrap();
...
#[cfg(feature = "xwing")]
protocol_tests!(shortcake::XWingSha3, xwing);

#[cfg(feature = "dhkem-p256")]
protocol_tests!(shortcake::DhkemP256Sha256, dhkem_p256);

#[cfg(feature = "dhkem-p384")]
protocol_tests!(shortcake::DhkemP384Sha384, dhkem_p384);
